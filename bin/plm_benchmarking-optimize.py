...
def n_models_trained(models_trained: list, ont: str):
    required_key = f"{ont.upper()} - Sort Score"
    count = 0
    for model in models_trained:
        if required_key in model:
            if model[required_key] > 0:
                count += 1
    return count

if __name__ == "__main__":
    '''python 32 120 80000 outputs/plm_benchmarking input_data/emb.ankh_base'''
    ...
    #if n_combinations < len(parameter_options):
    #    parameter_options = [p for i, p in enumerate(parameter_options) 
    #        if i < n_combinations]
    ...
    cc_bp_feature_cols = ['parti']
    deeploc_mf_feature_cols = ['parti', 'std', 'max']
    cc_bp_feature_descs = [f"{embs_parquet_prefix}_{c}.parquet:{c}" for c in cc_bp_feature_cols]
    deeploc_mf_feature_descs = [f"{embs_parquet_prefix}_{c}.parquet:{c}" for c in deeploc_mf_feature_cols]
    ...
    print(f"feature_descs={cc_bp_feature_descs}, {deeploc_mf_feature_descs}")
    ...
    for ont, datasets in datasets_by_ont.items():
        ...
        feature_descs = deeploc_mf_feature_descs if ont in ['deeploc', 'mf'] else cc_bp_feature_descs
    ...
    for config_i in range(len(parameter_options)):
        ...
        eval_metrics = {}
        for ont in ["deeploc", "mf", "cc", "bp"]:
            if n_models_trained(models_trained, ont) >= n_combinations:
                print(f"Skipping {ont} - already trained on {n_combinations} combinations")
                continue
